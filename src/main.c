				...
				TRY
					{
						...
						case INS_GET_SIGNED_PUBLIC_KEY: {
							...
							cx_hash(&hash.header, CX_LAST, publicKey.W, 65, result);
							tx += cx_ecdsa_sign((void*) &privateKey, CX_RND_RFC6979 | CX_LAST, CX_SHA256, result, sizeof(result), G_io_apdu_buffer + tx);
